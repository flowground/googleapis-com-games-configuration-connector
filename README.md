# ![LOGO](logo.png) Google Play Game Services Publishing **flow**ground Connector
...
* `maxResults` - _optional_ - The maximum number of resource configurations to return in the response, used for paging. For any response, the actual number of resources returned may be less than the specified maxResults.
...
*Tags:* `imageConfigurations`

#### Input Parameters
* `imageType` - _required_ - Selects which image in a resource for this method.
    Possible values: ACHIEVEMENT_ICON, LEADERBOARD_ICON.
* `resourceId` - _required_ - The ID of the resource used by this method.
* `key` - _optional_ - API key. Your API key identifies your project and provides you with API access, quota, and reports. Required unless you provide an OAuth 2.0 token.
* `oauth_token` - _optional_ - OAuth 2.0 token for the current user.
* `prettyPrint` - _optional_ - Returns response with indentations and line breaks.
* `quotaUser` - _optional_ - An opaque string that represents a user for quota purposes. Must not exceed 40 characters.
...
**flow**ground :- Telekom iPaaS / googleapis-com-games-configuration-connector<br/>
